...
class CheckDotFileTaskTest : ScenarioTest() {
  ...
  fun `Verify modules of a basic project`() = runScenario(GraphVizBasicWithThreeOutputFormats) {
    ...
    runTask(":a:generateModulesDotFile").build()
  ...
  @Test
  fun `Verify legend of a basic project`() = runScenario(GraphVizBasicWithBasicLegend) {
    // given initial dotfile is generated
    ...
    // given we set a custom property set to adjust the output
    resolve("gradle.properties").writeText("modular.graphViz.legend.cellPadding=10")
