...
internal fun writeD2Classes(config: D2ClassesConfig): String = buildIndentedString {
  appendStyles(config)
  appendVars(config)
...
private fun IndentedStringBuilder.appendClass(type: ProjectType) =
  with(type) {
    appendLine("$classId {")
    indent {
      val properties = mapOf("style.fill" to color) + type.properties(D2)
...
private fun linkAttributes(config: D2ClassesConfig, link: LinkType): List<Pair<String, String>> {
  ...
  attrs.putAll(link.properties(D2))
