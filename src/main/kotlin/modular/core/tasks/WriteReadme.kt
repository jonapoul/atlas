...
abstract class WriteReadme : DefaultTask(), ModularGenerationTask, TaskWithOutputFile {
  ...
  private fun StringBuilder.appendContents() {
    appendDiagram(tag = "chart", chartFile.get().asFile)
    appendLine()
    appendDiagram(tag = "legend", legendFile.get().asFile)
  }

  private fun StringBuilder.appendDiagram(tag: String, file: File) {
