...
/**
 * Represents a category of project that you can use to match against those in your project. You can use some of the
 ...
 * ```
 *
 * Remember that priority is given in descending order, so in the example above the UI project type will be checked
 * before the data or domain types.
 *
 * Exactly one of [ProjectTypeSpec.pathContains], [ProjectTypeSpec.pathMatches] or [ProjectTypeSpec.hasPluginId] must be
 * set. If not, Gradle will:
 * - warn you during IDE sync, or
 * - fail when running any tasks which reference them.
 */
@AtlasDsl
public interface NamedProjectTypeContainer<T : ProjectTypeSpec> : NamedDomainObjectContainer<T> {
...
@AtlasDsl
public interface ProjectTypeSpec : PropertiesSpec {
  /**
  ...
  /**
   * Checks against the path string of your project, e.g. ":path:to:my:project". This is case-sensitive.
   */
