...
internal class WriteMarkdownLegendTest : ScenarioTest() {
  @Test
  fun `Write markdown legend with no link types`() = runScenario(MermaidWithProjectTypes) {
    // when
  ...
  @Test
  fun `Write markdown legend with no project types`() = runScenario(MermaidWithLinkTypes) {
    // when
    val result = runTask(":writeMermaidLegend").build()
